...
For a Next.js site, that usually means rendering the document content with your preferred MDX tooling, such as `@next/mdx` , `next-mdx-remote-client`, `mdx-bundler`, `next-mdx-remote`, or `@mdx-js/mdx`.
