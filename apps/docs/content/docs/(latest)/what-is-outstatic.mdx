...
description: ''
publishedAt: 2026-06-03T02:58:52.391Z
status: draft
author:
  name: Andre Vitorio
...
Imagine we create a blog post titled: _How to raise a dragon_ - the full document path would be: `/outstatic/content/posts/how-to-raise-a-dragon.mdx`
...
But you are not limited to the above, you can also add [Custom Fields](/editing-modeling/custom-fields) to your documents.
...
Just like Collections, Singletons support [Custom Fields](/editing-modeling/custom-fields). Custom field schemas for singletons are stored as `{slug}.schema.json` in the `_singletons` directory.

Next: the [Content Editor](/editing-modeling/the-content-editor), which is used to edit Documents.
