...
For convenience we also store metadata in a `metadata.json` file to be used with our [Advanced Data Fetching](/access-integration/fetching-data#advanced-document-fetching---metadata-db) methods.
