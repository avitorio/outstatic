  ...
  for (const entry of entries) {
    ...
    if (fs.existsSync(schemaPath)) {
      try {
        ...
        console.warn(`Failed to parse schema for collection "${entry.name}":`, e)
      }
    }
...
function generateTypeFile(info: CollectionInfo): string {
  ...
  if (info.schema && Object.keys(info.schema.properties || {}).length > 0) {
    ...
    lines.push('')
    lines.push(`export interface ${info.interfaceName} extends OstDocument<${info.interfaceName}Fields> {}`)
  }

  lines.push('')
  lines.push(`export type ${info.interfaceName}Field = keyof ${info.interfaceName}`)
  lines.push('')
...
function generateCollectionsFile(
  ...
  for (const col of collections) {
    lines.push(`import type { ${col.interfaceName}, ${col.interfaceName}Fields } from './${col.slug}'`)
  }
...
function generateApiFile(
  ...
  lines.push(`// Generated by Outstatic - do not edit manually`)
  lines.push(`// This file provides typed overloads for Outstatic server functions`)
  lines.push('')
  lines.push(`import type { CollectionName, CollectionDocument, SingletonName, SingletonDocument, AllDocuments } from './collections'`)

  // Import Fields types for each collection
  for (const col of collections) {
    lines.push(`import type { ${col.interfaceName}Fields } from './${col.slug}'`)
  }

  // Import Fields types for each singleton
  for (const singleton of singletons) {
    lines.push(`import type { ${singleton.interfaceName}Fields } from './_singletons/${singleton.slug}'`)
  }
  ...
  if (hasCollections) {
    lines.push(`/** Infers the document type from a query's collection property */`)
    lines.push(`type InferDocumentType<Q> =`)
    // Generate explicit checks for each collection to ensure literal types are preserved
    for (const col of collections) {
      lines.push(`  Q extends { collection: '${col.slug}' } ? OutstaticSchema<${col.interfaceName}Fields> :`)
    }
  ...
  if (hasCollections) {
    ...
    for (const col of collections) {
      ...
      lines.push(`  /** Find ${col.slug} documents with field projection (use 'as const' for type narrowing) */`)
      lines.push(`  find<K extends keyof OutstaticSchema<${col.interfaceName}Fields>>(`)
      lines.push(`    query: { collection: '${col.slug}' } & Record<string, unknown>,`)
      lines.push(`    projection: readonly K[]`)
      lines.push(`  ): FindAPI<OutstaticSchema<${col.interfaceName}Fields>, Pick<OutstaticSchema<${col.interfaceName}Fields>, K>>`)
      lines.push(``)
    ...
    lines.push(`  /** Find documents with field projection (use 'as const' for type narrowing) */`)
    lines.push(`  find<K extends keyof OutstaticSchema<AllDocuments>>(`)
    lines.push(`    query: Record<string, unknown>,`)
    lines.push(`    projection: readonly K[]`)
    lines.push(`  ): FindAPI<OutstaticSchema<AllDocuments>, Pick<OutstaticSchema<AllDocuments>, K>>`)
    lines.push(``)
  ...
  if (hasCollections) {
    ...
    lines.push(`   * @param fields - Optional array of fields to include (use 'as const' for type narrowing)`)
    lines.push(`   */`)
    lines.push(`  export function getDocuments<`)
    lines.push(`    C extends CollectionName,`)
    lines.push(`    F extends readonly (keyof CollectionDocument<C>)[] | undefined = undefined`)
    lines.push(`  >(`)
    ...
    lines.push(`   * @param collection - The collection name`)
    lines.push(`   */`)
    lines.push(`  export function getDocumentSlugs(collection: CollectionName): string[]`)
    lines.push('')
    ...
    lines.push(`   * @param collection - The collection name`)
    lines.push(`   */`)
    lines.push(`  export function getDocumentPaths(collection: CollectionName): { params: { slug: string } }[]`)
    lines.push('')
  }
  ...
  if (hasSingletons) {
    ...
    lines.push(`    F extends readonly (keyof SingletonDocument<S>)[] | undefined = undefined`)
    lines.push(`  >(`)
    lines.push(`    slug: S,`)
    lines.push(`    fields?: F`)
  ...
  lines.push(`   * @returns LoadAPI with typed find method that narrows based on collection query`)
  lines.push(`   */`)
...
export function generateTypes(options: GenerateTypesOptions = {}): void {
  ...
  const contentPath = path.join(cwd, options.contentPath || DEFAULT_CONTENT_PATH)
